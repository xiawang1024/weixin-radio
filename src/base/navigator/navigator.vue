  ...
  <div ref="viewport" class="navigator-component">
    <scroll ref="scroll" direction="horizontal">
      <ul class="tab-list" ref="tabList">
        <li v-for="(item, index) in navList" class="tab-item" @click="selectNav(item)" :key="item.date">
          <slot name="item" :week="item.week" :date="item.date" :index="index">
            <span class="tab-name" :class="{'link-active':isCurrentTab(item.id)}">{{item.date}}</span>
          </slot>
        </li>
      </ul>
    </scroll>
